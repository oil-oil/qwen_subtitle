...
| 配置名 | 业务环境变量 | 系统凭据引用 |
| --- | --- | --- |
| default | `DASHSCOPE_API_KEY` | `qwen-subtitle/dashscope/default` |
...
```bash
node "$SKILL_DIR/scripts/credential-ui/src/profile.ts" run default -- python3 "$SKILL_DIR/scripts/dub_multi.py" --help
```
...
纠错、翻译、上传和合成仍使用已登录的 `bl`，通过 `bl auth status` 核对，并复用官方授权入口。页面只为脚本直接调用的声音复刻 API 提供 `DASHSCOPE_API_KEY`，不会替代或修改 CLI 登录。已有 CLI 凭据可用时不重复填写。只有用户明确要求并授权声音使用时才运行配音分支。
