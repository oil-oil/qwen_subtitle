...
SCRIPT = ROOT / 'scripts/dub_multi.py'
...
class BusinessCredentialTests(unittest.TestCase):
    def test_profile_environment_reaches_business_reader(self):
        with patch.dict(os.environ, {'DASHSCOPE_API_KEY': 'TEST_KEY'}, clear=True), patch.object(Path,'read_text',side_effect=AssertionError('不应读取旧凭据文件')):
            self.assertEqual(module.dashscope_key(), 'TEST_KEY')
