---
name: qwen-subtitle
description: "使用百炼模型为视频生成和纠正字幕，结合真实画面核对术语，按需翻译字幕或克隆授权声音配音。用户明确要求本 Skill、百炼模型自动纠错或多语言配音时使用。普通中文成片字幕优先使用 oil-subtitle；不用于 Screen Studio 工程剪辑。只要求翻译或英文字幕时不克隆、不配音；克隆声音必须有明确使用授权。"
---
